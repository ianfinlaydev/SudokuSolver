...
{
    internal class SectionNonet : SectionBase
    {
        internal SectionNonet(List<SectionBase> sections, int[,] elements, (int row, int column) sectionCoords) : base(sections, elements, sectionCoords)
        {
            _sectionDimensions = (3, 3);
                ...
                if (_emptyElementCoords.Count <= 4)
                {
                    for (int i = _emptyElementCoords.Count - 1; i >= 0; i--)
                    {
                        ...
                        candidateValues.RemoveAll(
                            value => RowContains(value, _emptyElementCoords[i].Row) || ColumnContains(value, _emptyElementCoords[i].Column));
